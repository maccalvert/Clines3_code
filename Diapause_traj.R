...
##Checked again for consitency prior to submission to online repo
## MBC 2/4/2020

#Note: The ggplot functions displaying the diapause metabolic rate trajectorires will print a warning that 
#it has removed rows with missing data. There is notion wrong, those rows indicate samples that died or 
# emerged during the experiment. 

setwd('/media/raglandlab/ExtraDrive4/Clines_3/')
dataAH<-read.table('./data/HawAppleAllTrajectories_R.txt',sep="\t",header=T,row.names=NULL)
...
# User Z. lin's answer. 
library(ggplot2)
pdf("./results/Diapause_traj_mosaic_2008.pdf")
...
pdf("./results/Diapause_traj_appleTraj_2008.pdf")
...
pdf("./results/Diapause_traj_hawTraj_2008.pdf")
...
traj_2009 <- read.table('./data/AllLiveFlys_R.txt',sep="\t",header=T,row.names=NULL)
...
pdf("./results/Diapause_traj_hawTraj_2009_supp_fig_1.pdf")
...
eclData<-read.table('./data/GlenPreWinterExp2009_R.txt',sep="\t",header=T,row.names=NULL)
...
library(ggplot2)
pdf("./results/Diapause_Traj_Eclosion_supp_fig2.pdf")
